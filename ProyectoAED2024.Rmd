...
datos_filtrados <- na.omit(datos_CNAE[datos_CNAE$Sexo == "Ambos sexos" &
...
grafico_treemap <- ggplot(datos_filtrados, aes(area = Total_porc, fill = Total_porc, label = paste(datos_filtrados$Rama.abreviada, Total_porc, sep = "\n"))) +
  geom_treemap() +
  geom_treemap_text(aes(label = ifelse(Total_porc > 1, paste(datos_filtrados$Rama.abreviada, Total_porc, sep = "\n"), "")), 
                    colour = "white", 
                    place = "centre", 
                    size = 8) + 
...
ggplot(datos_CNAE[datos_CNAE$Sexo != "Total" & datos_CNAE$Rama.de.actividad.CNAE.2009 == "Total", ], 
...
```{r, warning=FALSE, echo=FALSE}
ggplot(datos_CNAE[datos_CNAE$Rama.de.actividad.CNAE.2009=="99 Actividades de organizaciones y organismos extraterritoriales",], aes(x=Periodo,y=Total_abs, color=Sexo))+geom_line()+labs(title="Evolución del empleo en 'Actividades de orgs. y organismos extraterrit.'")
ggplot(datos_CNAE[datos_CNAE$Rama.de.actividad.CNAE.2009=="07 Extracción de minerales metálicos",], aes(x=Periodo,y=Total_abs, color=Sexo))+geom_line()+labs(title="Evolución del empleo en 'Extracción de minerales metálicos'")
```
...
medias_por_cluster <- data_wide %>%
  ...
  pivot_longer(-c(Cluster,Rama.de.actividad.CNAE.2009), names_to = "Periodo", values_to = "Media")

grafico_lineas <- ggplot(medias_por_cluster, aes(x = Periodo, y = 100*Media, color = Cluster, group = Cluster)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
...
grafico_cuantiles <- ggplot(cuantiles_por_cluster, aes(x = Periodo, y = Valor, color = Cluster, group = interaction(Cluster, Cuantil))) +
  geom_line(size = 1) +
  geom_point(size = 2) +
